...
parse_frame_bin(<<"BUF">>, _Size, <<0:8, BufferSize:24/integer, 1:8/integer, NextFlagOffset:8/integer>>) ->
  {buf, [{buffer_size, BufferSize}, {embedded_info, true}, {next_flag_offset, NextFlagOffset}]};

parse_frame_bin(<<"BUF">>, _Size, <<0:8, BufferSize:24/integer, 0:8/integer>>) ->
...
parse_frame_bin(<<"COM">>, _Size, FrameContent) ->
  case parse_text_string(FrameContent) of
    {Lang, none, Comment} ->
      {com, [{language, Lang}, {comment, Comment}]};
    {Lang, ShortDesc, Comment} ->
      {com, [{language, Lang}, {short_description, ShortDesc}, {comment, Comment}]}
  end;
...
parse_text_string(<<Enc:8/integer, Language:3/binary, Rest/binary>>) ->
  case binary:match(Rest, <<0>>) of
    {Start, _Len} ->
      {ShortDesc, C} = split_binary(Rest, Start),
      <<0:8/integer, Comment/binary>> = C,
      {utils:decode_string(Enc, Language), utils:decode_string(Enc, ShortDesc), utils:decode_string(Enc, Comment)};
    nomatch ->
      {utils:decode_string(Enc, Language), none, binary_to_list(Rest)}
  end.